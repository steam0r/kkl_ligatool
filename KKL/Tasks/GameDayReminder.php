...
class KKL_Tasks_GameDayReminder {
  ...
  public static function execute() {
    ...
    if (!empty($matches)) {
      ...
      KKL_Events_Service::fireEvent(
        KKL_Events_Service::$NEW_GAMEDAY_UPCOMING,
        new KKL_Events_GameDayReminderEvent($matches, $topMatches, $offset)
      );
    }
  }
  ...
  private static function getMatches($days_off) {
    ...
    $kkl_db_tool = new KKL_DB_Wordpress();
    ...
    $sql = "select l.name as league, s.id as season_id, g.number as day_number, l.code as leaguecode, home.name as home, away.name as away, home.id as home_id, away.id as away_id, loc.title as location, CONCAT(home.name,' gg. ', away.name) as title, slack.value as slack " .
      "from leagues as l " .
      "JOIN seasons as s ON s.id = l.current_season " .
      "JOIN game_days AS g ON g.id = s.current_game_day " .
      "JOIN game_days AS ug ON ug.season_id = s.id and ug.number = g.number + 1 AND date(ug.fixture) = date(NOW() + INTERVAL " . $days_off . " DAY) " .
      "JOIN `matches` AS m ON m.game_day_id = ug.id " .
      "JOIN teams as home ON m.home_team = home.id " .
      "JOIN teams AS away ON m.away_team = away.id " .
      "LEFT JOIN team_properties AS p ON p.objectId = home.id AND p.property_key = 'location' " .
      "LEFT JOIN locations AS loc ON loc.id = p.value " .
  ...
  private static function getTopMatches($matches) {
    ...
    foreach ($days as $season => $day) {
      ...
      $potentialMatches[$seasons[$season]['name']] = array(
        "top" => array(
          $first['team_id'],
          $second['team_id']
        ),
