...
class Shortcodes {
  ...
  public static function leagueOverview($attrs, $content, $tag) {
    ...
    foreach ($all_leagues as $league) {
      $league->season = $db->getSeason($league->getCurrentSeason());
      $league->teams = $db->getTeamsForSeason($league->season->getId());
      foreach ($league->teams as $team) {
        $club = $db->getClub($team->getClubId());
        if (!$team->getLogo()) {
      ...
      $day = $db->getGameDay($league->season->getCurrentGameDay());
  ...
  public static function leagueTable($atts, $content, $tag) {
    ...
    $properties = $db->getSeasonProperties($context['season']->getId());
  ...
  public static function tableOverview($atts, $content, $tag) {
    ...
    foreach ($db->getActiveLeagues() as $league) {
      ...
      $day = $db->getGameDay($season->getCurrentGameDay());
      $ranking = new stdClass;
  ...
  public static function clubDetail($atts, $content, $tag) {
    ...
    foreach ($seasonTeams as $seasonTeam) {
      ...
      $seasonTeam->season->league = $db->getLeague($seasonTeam->season->getLeagueId());
  ...
  public static function gameDayPager($atts, $content, $tag) {
    ...
    if (!$next) {
      ...
      $next->link = Plugin::getLink('league', array('league' => $league->code, 'season' => date('Y', strtotime($season->start_date)), 'game_day' => $next->getNumber()));
    }
